...
def perform_functional_test_of_file(filename):
    ...
    # Copy the Makefile and sales.c
    subprocess.call(["cp", "Makefile", "."])
    subprocess.call(["cp", "sales.c", "."])
    status = "Pass", 1
    
    try:
        compile_c_file(filename, additonal_args=["sales.c"])
        output = subprocess.run(["./a.out"], stdout=subprocess.PIPE, timeout=5).stdout.decode('utf-8')
